...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .logo { font-size: 24px; font-weight: 900; color: #333; }
    .os-numero { font-size: 18px; font-weight: bold; }
    .titulo { font-weight: bold; font-size: 12px; margin: 20px 0 10px; text-transform: uppercase; border-left: 3px solid #333; padding-left: 8px; }
    .grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 15px; line-height: 1.8; }
    .campo label { font-weight: bold; }
    .footer { margin-top: 50px; border-top: 1px solid #ccc; padding-top: 12px; font-size: 10px; color: #888; text-align: center; }
  </style>
</head>
<body>
  <div class="header">
    <div>
      <div class="logo">brooks AMBIENTAL</div>
      <div style="font-size:10px; color:#666;">Ordem de Serviço</div>
    </div>
    <div style="text-align:right;">
      <div class="os-numero">OS #${orden.id.slice(0,8).toUpperCase()}</div>
      <div style="font-size:11px;">${hoje}</div>
    </div>
  </div>

  <div class="titulo">Dados do Cliente</div>
  <div class="grid2">
    <div class="campo"><label>Empresa: </label>${cliente.nombre}</div>
    <div class="campo"><label>CNPJ/CPF: </label>${cliente.documentoFiscal}</div>
    <div class="campo"><label>Endereço: </label>${cliente.direccion || '—'}</div>
    <div class="campo"><label>Segmento: </label>${cliente.segmento || '—'}</div>
  </div>

  <div class="titulo">Dados do Serviço</div>
  <div class="grid2">
    <div class="campo"><label>Tipo: </label>${orden.tipoServicio}</div>
    <div class="campo"><label>Frequência: </label>${orden.frecuenciaServicio}</div>
    <div class="campo"><label>Endereço coleta: </label>${orden.direccionServicio}</div>
    <div class="campo"><label>Unidade operacional: </label>${orden.unidadeOperacional || '—'}</div>
    ${orden.dataExecucao ? `<div class="campo"><label>Data execução: </label>${new Date(orden.dataExecucao).toLocaleDateString('pt-BR')}</div>` : ''}
  </div>

  ${orden.observaciones ? `
  <div class="titulo">Observações Operacionais</div>
  <p style="line-height:1.7; margin-top:8px;">${orden.observaciones}</p>` : ''}
